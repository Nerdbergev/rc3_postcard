    ...
    for uid, message_data in server.fetch(messages, 'RFC822').items():
        ...
        for part in email_message.walk():
            if part.get_content_type() == 'image/jpeg':
                ...
                if bool(fileName):
                    filePath = os.path.join(os.path.dirname(os.path.abspath(__file__)) + '/cards/', fileName)
                    if not os.path.isfile(filePath):
                        fp = open(filePath, 'wb')
                        fp.write(part.get_payload(decode=True))
                        fp.close()
                crop(os.path.dirname(os.path.abspath(__file__)) + '/cards/' + fileName, part.get_payload(decode=True))
